...
const parseMarksLine = (line: string): string[] => {
  ...
  while (i < parts.length) {
    ...
    // This perfectly parses ATKT marks with plus signs
...
const PdfResultConverter: React.FC = () => {
  ...
  const handleDownloadCsv = () => {
    ...
    a.href = url; a.download = "student_results_dynamic.csv"; a.click();
    URL.revokeObjectURL(url);
  };

  const handleUploadToDatabase = async () => {
    if (!semester) return alert("Please enter a Semester number before uploading.");
    ...
    try {
      // Dynamic Endpoint based on Checkbox
  ...
  const handleNepUpload = async () => {
    if (!nepFile || !semester) return alert("Please select a PDF or CSV file and enter the semester number.");
    setIsProcessing(true); setAllStudents([]);
    const formData = new FormData();
    formData.append("file", nepFile); formData.append("semester", semester);

    try {
      const response = await fetch("http://localhost:5000/api/students/upload-nep-pdf", { method: "POST", body: formData });
      const result = await response.json();
      if (response.ok) {
        alert("Success: " + result.message);
        if (result.students) setAllStudents(result.students);
      } else {
        alert("NEP Upload Failed: " + (result.error || "Server error"));
      }
    } catch (error) { alert("Network error during NEP upload."); } 
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4 space-y-8">
      ...
      <div className="border rounded-lg p-6 space-y-6 bg-white shadow-sm border-indigo-100">
        <h2 className="text-xl font-bold text-indigo-800 border-b pb-2">R-19 Client-Side Processor</h2>
        ...
        <div className="space-y-2 pt-4 border-t border-gray-200">
          ...
          {/* THE NEW ATKT CHECKBOX */}
          <div className="flex items-center space-x-2 mb-4 bg-gray-50 p-3 rounded-lg border border-gray-200">
            <input 
              type="checkbox" id="isAtkt" 
              checked={isATKT} onChange={(e) => setIsATKT(e.target.checked)} 
              className="w-5 h-5 text-indigo-600 rounded cursor-pointer"
            />
            <label htmlFor="isAtkt" className="text-sm font-bold text-gray-700 cursor-pointer select-none">
              This is an ATKT Result (Smartly updates existing records)
            </label>
          </div>

          <div className="flex gap-4">
            <input type="number" placeholder="Semester (e.g. 1)" value={semester} onChange={(e) => setSemester(e.target.value)} className="p-2 border rounded-md w-32 text-sm" min="1" max="8" />
            <button
              onClick={handleUploadToDatabase} disabled={!hasData || isProcessing || isUploading}
              className={`flex-1 text-white font-bold py-2 px-4 rounded-lg transition-colors ${isUploading ? "bg-gray-400 cursor-not-allowed" : "bg-green-600 hover:bg-green-700"}`}
            >
              {isUploading ? "Uploading..." : `Upload ${isATKT ? 'ATKT' : 'CSV'} to DB`}
            </button>
          </div>
          <p className="text-xs text-gray-500">* Converts extracted data to CSV and saves to MongoDB.</p>
        </div>
      </div>

      {/* --- NEP 2024 SECTION --- */}
      <div className="border rounded-lg p-6 space-y-6 bg-white shadow-sm border-purple-100">
        <h2 className="text-xl font-bold text-purple-800 border-b pb-2">NEP 2024 Direct Database Upload</h2>
        <div className="space-y-4">
          <div className="space-y-2">
            <label className="text-sm font-medium">Semester Number</label>
            <input type="number" placeholder="Enter Semester (e.g. 1)" value={semester} onChange={(e) => setSemester(e.target.value)} className="w-full p-2 border rounded-md text-sm" min="1" max="8" />
          </div>
          <div className="space-y-2">
            <label className="text-sm font-medium">Upload NEP Result (CSV or PDF)</label>
            <input
              type="file" accept=".pdf,.csv" onChange={(e) => setNepFile(e.target.files?.[0] || null)}
          ...
          <button
            ...
            className="w-full bg-purple-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-purple-700 disabled:bg-gray-400"
          >
            {isProcessing ? "Uploading..." : "Upload to NEP Database"}
          </button>
          <p className="text-xs text-gray-500 mt-2">* Uploads bypass the client parser and go straight to the database.</p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="border rounded-lg p-6 bg-white shadow-sm">
          <h2 className="text-lg font-semibold mb-4">Raw Extracted Text (R-19)</h2>
        ...
        <div className="border rounded-lg p-6 bg-white shadow-sm">
          <h2 className="text-lg font-semibold mb-4">Cleaned Text for Parsing (R-19)</h2>
      ...
      {/* --- COMBINED DATA PREVIEW TABLE --- */}
      <div className="border rounded-lg p-6 bg-white shadow-sm">
        <h2 className="text-lg font-semibold mb-4">
          Final Parsed Data Preview ({allStudents.length} students loaded)
        </h2>
          ...
          <table className="min-w-full text-sm text-left">
            ...
            <tbody className="divide-y">
              {hasData ? (
                ...
                <tr>
                  <td colSpan={5} className="h-24 text-center">No data to display. Upload a file to begin.</td>
                </tr>
              )}
            </tbody>
          </table>
