...
const Sem3Analysis = () => {
  ...
  const [selectedBatch, setSelectedBatch] = useState(""); // Catches smart backend default
  const [isLoading, setIsLoading] = useState(true);
  const lastFetchedBatch = useRef(null);

  // Mapped strictly to Sem 3 Core Theory Subjects
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/students/sem/3${queryParam}`)
      .then((res) => res.json())
      .then((res) => {
        setData(res);
    ...
    return (
      <div className="flex justify-center items-center h-screen bg-slate-50">
        <div className="p-10 text-center text-blue-600 font-bold text-xl animate-pulse">
          Loading Official Gazette (Sem 3)...
        </div>
      </div>
    );
  }
  ...
  // Clean UI mapping for official S.E. Sem 3 Subjects
  const cleanSubjectNames = {
    EM3_TOT_Marks: "Engg Mathematics-III",
    DSA_TOT_Marks: "Data Structures (DSA)",
    DBMS_TOT_Marks: "Database Mgmt (DBMS)",
    PC_TOT_Marks: "Principle of Comm (PC)",
    PCPF_TOT_Marks: "P.C.P.F.",
  };
        ...
        <>
          ...
          <div className="text-center mb-8 leading-tight">
            ...
            <h3 className="text-xl font-bold text-slate-900">
              RESULT ANALYSIS S.E. SEMESTER III (BATCH: {selectedBatch})
            </h3>
                    ...
                    <tr key={idx} className="hover:bg-slate-50 group">
                      ...
                      <td className="border border-black p-0 text-left relative">
                        <input
                          ...
                          className="w-full h-full px-3 py-2 bg-transparent focus:outline-none focus:bg-yellow-50 font-medium text-slate-900"
