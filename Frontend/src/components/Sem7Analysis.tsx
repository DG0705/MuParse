...
const useDownloadReport = (
  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      const analysisHtml = getFullHtmlContent(
        subjectAnalysisRef,
        "RESULT ANALYSIS B.E. SEM VII (Subjectwise Report)",
      );
      if (analysisHtml) {
      ...
      const summaryHtml = getFullHtmlContent(
        overallSummaryRef,
        "RESULT ANALYSIS B.E. SEM VII (Summary & Toppers)",
      );
      if (summaryHtml) {
      ...
      pdf.save("Result_Analysis_Combined_Tall_Report_Sem7.pdf");
    } catch (error) {
  ...
  const downloadPng = async () => {
    ...
    try {
      ...
      link.download = "Result_Analysis_Combined_Sem7.png";
...
const Sem7Analysis: React.FC = () => {
  ...
  const summary: SummaryData = useMemo(() => {
    ...
    subjectKeys.forEach((markKey) => {
      // Clean string for UI display by stripping out "_Marks"
      const subjectName = markKey.replace(/_Marks$/g, "").replace(/_/g, " ");
      ...
      const passMark = 40; // Default passing mark for Sem 7 Subjects
      ...
      subjectAnalysis[subjectName] = {
        ...
        marks51_59,
        marks60_Above,
        teacher: "N/A", // Teachers can be mapped dynamically if needed
      };
    });
      ...
      <div className="text-center mb-6 pb-4 border-b border-dashed border-gray-300">
        ...
        {!loading && parsedData.length === 0 && !error && (
          <div className="bg-yellow-50 border border-yellow-200 p-4 rounded text-yellow-800">
            <p>No records found in the database for Semester VII.</p>
          </div>
        )}
                  ...
                  <tr className="bg-red-50">
                    ...
                    {summary.subjectKeys.map((key) => (
                      <th key={key} className="p-2 border border-gray-300">
                        {key.replace(/_Marks$/g, "").replace(/_/g, " ")}
                      </th>
                    ))}
