...
const Sem7Analysis = () => {
  ...
  const [selectedBatch, setSelectedBatch] = useState("2022"); // Catches smart backend default
  const [isLoading, setIsLoading] = useState(true);
  const lastFetchedBatch = useRef(null);

  // Mapped strictly to your custom Sem 7 Faculty list
  const [teachers, setTeachers] = useState({
    ...
    AI_DS_II_Marks: "Dr. Swati Narwane",
    IoE_Marks: "Prof. Snehal Bhure",
    Data_Science_Lab_Marks: "",
    Major_Project_I_Marks: "",
    Mgmt_Info_Sys_Marks: "Prof. A.E Patil",
    Infra_Security_Marks: "Dr. Sunil Wankhade",
    Info_Retrieval_Sys_Marks: "Prof. Rahul Jiwane",
    Cyber_Security_Laws_Marks: "Prof. Manisha Ahira",
    Software_Testing_QA_Marks: "Dr. Swati Narwane",
  });
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/students/sem/7${queryParam}`)
      .then((res) => res.json())
      .then((res) => {
        setData(res);
    ...
    return (
      <div className="flex justify-center items-center h-screen bg-slate-50">
        <div className="p-10 text-center text-blue-600 font-bold text-xl animate-pulse">
          Loading Official Gazette (Sem 7)...
        </div>
      </div>
    );
  }
  ...
  // Clean UI mapping for B.E. Semester VII (IT)
        ...
        <>
          ...
          <div className="text-center mb-8 leading-tight">
            ...
            <h3 className="text-xl font-bold text-slate-900">
              RESULT ANALYSIS B.E. SEMESTER VII (BATCH: {selectedBatch})
            </h3>
