...
const Sem1Analysis = () => {
  ...
  const [selectedBatch, setSelectedBatch] = useState(""); // Starts empty
  ...
  useEffect(() => {
    // Prevent React from re-triggering an identical fetch
    ...
    fetch(`http://localhost:5000/api/students/sem/1${queryParam}`)
      .then((res) => res.json())
      .then((res) => {
        setData(res);

        // ---> THE PATCH: Forcibly sync the memory ref, even on a 404! <---
        lastFetchedBatch.current = selectedBatch || res.selectedBatch;
  ...
  const cleanSubjectNames = {
    EM1_TOT_Marks: "Engg Mathematics-I",
    EP1_TOT_Marks: "Engg Physics-I",
    EC1_TOT_Marks: "Engg Chemistry-I",
    EMEC_TOT_Marks: "Engg Mechanics",
    BEE_TOT_Marks: "B.E.E.",
  };
        ...
        <>
          ...
          <div className="text-center mb-8 leading-tight">
            ...
            <h3 className="text-xl font-bold text-gray-900">
              RESULT ANALYSIS F.E. SEMESTER I (BATCH: {selectedBatch})
            </h3>
