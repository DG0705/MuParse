// Frontend/src/components/SemesterAnalysis.tsx
...
// --- Props Interface ---
interface SemesterAnalysisProps {
  semesterNumber: number;
  reportTitle: string;
}

const SubjectAnalysisReport: React.FC<{ analysisData: AnalysisData }> = ({ analysisData }) => {
  const chartData = Object.entries(analysisData).map(([subject, stats]) => ({
    subject: subject.split(" ").map((s) => s.substring(0, 3)).join(""),
  ...
  const maxBarValue = Math.max(...Object.values(analysisData).map((s) => s.totalAppeared), 1);
  ...
  return (
    <div className="p-4 border border-gray-400 mt-8">
      <h2 className="text-xl font-bold text-center mb-4">SUBJECT-WISE RESULT SUMMARY</h2>
      <p className="text-sm text-center text-gray-500 mb-4 italic">
        * You can click on the teacher's name to edit it before downloading.
      </p>
        ...
        <table className="min-w-full border border-black text-xs text-center">
          <thead>
            <tr className="bg-blue-50">
              <th rowSpan={2} className="p-2 border border-black font-bold">SUBJECT</th>
              <th rowSpan={2} className="p-2 border border-black font-bold">NAME OF TEACHER</th>
              <th rowSpan={2} className="p-2 border border-black font-bold">TOTAL STUDENTS APPEARED</th>
              <th rowSpan={2} className="p-2 border border-black font-bold">TOTAL STUDENTS PASSED</th>
              <th rowSpan={2} className="p-2 border border-black font-bold">PASS PERCENTAGE</th>
              <th colSpan={3} className="p-2 border border-black font-bold">CANDIDATES SCORING</th>
            </tr>
            <tr className="bg-blue-50">
              ...
              <th className="p-2 border border-black font-bold">60 & ABOVE MARKS</th>
            </tr>
          </thead>
          <tbody>
            {Object.entries(analysisData).map(([subject, stats]) => (
              <tr key={subject} className="even:bg-gray-50">
                <td className="p-2 border border-black font-semibold">{subject}</td>
                <td 
                  className="p-2 border border-black outline-none cursor-text focus:bg-blue-100 transition-colors"
                  contentEditable
                ...
                <td className="p-2 border border-black">{stats.totalAppeared}</td>
                <td className="p-2 border border-black">{stats.totalPassed}</td>
                <td className="p-2 border border-black font-bold text-green-700">{stats.passPercentage}</td>
                <td className="p-2 border border-black">{stats.marks40_50}</td>
                <td className="p-2 border border-black">{stats.marks51_59}</td>
                <td className="p-2 border border-black">{stats.marks60_Above}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="grid grid-cols-1 gap-10">
        <div className="border border-gray-400 p-4 pt-10 h-[400px]">
          <h4 className="text-center font-bold mb-4">Result Analysis (Appeared vs. Passed)</h4>
          <ResponsiveContainer width="100%" height="90%">
            <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              <Legend />
              <Bar dataKey="Appeared" fill="#3B82F6" name="Total Students Appeared" />
              <Bar dataKey="Passed" fill="#10B981" name="Total Students Passed" />
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div className="border border-gray-400 p-4 pt-10 h-[400px]">
          <h4 className="text-center font-bold mb-4">Subject-wise Statistical Analysis (Marks Breakdown)</h4>
          <ResponsiveContainer width="100%" height="90%">
            <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} />
              <XAxis dataKey="subject" />
...
const useDownloadReport = (
  ...
  const getFullHtmlContent = (contentRef: React.RefObject<HTMLDivElement>, title: string) => {
  ...
  const html2canvasOptions = { scale: 2.5, useCORS: true, allowTaint: true, svg: true, timeout: 15000, logging: false } as any;
  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      const pdf = new jsPDF("p", "mm", [210, 1000]);
      ...
      const analysisContentNode = getFullHtmlContent(subjectAnalysisRef, `${reportTitle} (Subjectwise Report)`);
      if (analysisContentNode) {
        analysisContentNode.childNodes.forEach((node) => combinedContent.appendChild(node.cloneNode(true)));
      }

      const separator = document.createElement("div");
      separator.style.cssText = "height: 20px; border-top: 2px solid #333; margin: 20px 0;";
      combinedContent.appendChild(separator);

      const summaryContentNode = getFullHtmlContent(overallSummaryRef, `${reportTitle} (Summary & Toppers)`);
      if (summaryContentNode) {
        summaryContentNode.childNodes.forEach((node) => combinedContent.appendChild(node.cloneNode(true)));
      }
      ...
      pdf.save(`Result_Analysis_Sem_${reportTitle.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`);
      ...
      if (analysisHtml && document.body.contains(analysisHtml)) document.body.removeChild(analysisHtml);
  ...
  const downloadPng = async () => {
    ...
    try {
      ...
      const page1Html = getFullHtmlContent(subjectAnalysisRef, `${reportTitle} (Subjectwise Report)`);
      if (page1Html) page1Html.childNodes.forEach((node) => combinedContainer?.appendChild(node.cloneNode(true)));

      const separator = document.createElement("div");
      separator.style.cssText = "height: 20px; border-top: 1px dashed #ccc; margin: 20px 0;";
      combinedContainer.appendChild(separator);

      const page2Html = getFullHtmlContent(overallSummaryRef, `${reportTitle} (Summary & Toppers)`);
      if (page2Html) page2Html.childNodes.forEach((node) => combinedContainer?.appendChild(node.cloneNode(true)));
      ...
      link.download = `Result_Analysis_Sem_${reportTitle.replace(/[^a-zA-Z0-9]/g, '_')}.png`;
      ...
      toast({ title: "PNG Generation Failed", description: "Failed to generate PNG.", variant: "destructive" });
    } finally {
      if (combinedContainer && document.body.contains(combinedContainer)) document.body.removeChild(combinedContainer);
      setIsDownloading(false);
    }
  };
...
const SemesterAnalysis: React.FC<SemesterAnalysisProps> = ({ semesterNumber, reportTitle }) => {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        setLoading(true);
        // DYNAMIC SEMESTER NUMBER USED HERE
        const url = `http://localhost:5000/api/students/analytics/semester-batch?semester=${semesterNumber}&batchYear=${selectedYear}`;
    ...
    fetchStudents();
  }, [selectedYear, semesterNumber]);

  const summary: SummaryData = useMemo(() => {
    if (parsedData.length === 0) {
      return {
        totalStudents: 0, successful: 0, passPercentage: "0.00%",
        overallToppers: [], subjectToppers: {},
        malePassed: 0, maleFailed: 0, femalePassed: 0, femaleFailed: 0,
        subjectAnalysis: {}, processedData: [], subjectMapping: {},
      };
    }
    ...
    const isStatisticallySuccessful = (student: StudentData): boolean => {
      if (student.Result?.trim().toUpperCase() !== "P" || student.Remark?.trim().toUpperCase() === "RLE") return false;
    ...
    const getFinalStatus = (student: StudentData): "Passed" | "Failed" => {
      return student.Result?.trim().toUpperCase() === "F" || student.Remark?.trim().toUpperCase() === "RLE" ? "Failed" : "Passed";
    };
    ...
    const successfulStudents = processedData.filter((s) => s.isStatisticallySuccessful);
    const maleStudents = processedData.filter((s) => s.Gender === "Male");
    const femaleStudents = processedData.filter((s) => s.Gender === "Female");
    const malePassed = maleStudents.filter((s) => s.isStatisticallySuccessful).length;
    const femalePassed = femaleStudents.filter((s) => s.isStatisticallySuccessful).length;
    ...
    subjectKeys.forEach((markKey) => {
      ...
      const studentsWithMarks = processedData.filter((s) => s[markKey] && !isNaN(cleanAndParseInt(s[markKey])));
      const totalAppeared = studentsWithMarks.length;
      const totalPassed = studentsWithMarks.filter((s) => cleanAndParseInt(s[markKey]) >= 40).length;

      subjectAnalysis[subjectName] = {
        totalAppeared,
        totalPassed,
        passPercentage: totalAppeared > 0 ? ((totalPassed / totalAppeared) * 100).toFixed(2) + "%" : "0.00%",
      ...
      if (studentsWithMarks.length > 0) {
        const sortedStudents = [...studentsWithMarks].sort((a, b) => cleanAndParseInt(b[markKey]) - cleanAndParseInt(a[markKey]));
        ...
        for (const student of sortedStudents) {
          ...
          if (uniqueScores.size < 2 || uniqueScores.has(currentMark)) {
            toppersList.push({ name: student.Name, marks: currentMark, seatNo: student.Seat_No });
            uniqueScores.add(currentMark);
          } else if (uniqueScores.size === 2 && currentMark < toppersList[toppersList.length - 1].marks) {
            break;
          }
        }
    ...
    const passPercentage = totalStudents > 0 ? ((successful / totalStudents) * 100).toFixed(2) : "0.00";

    return {
      totalStudents, successful, passPercentage,
      overallToppers, subjectToppers,
      malePassed, maleFailed: maleStudents.length - malePassed,
      femalePassed, femaleFailed: femaleStudents.length - femalePassed,
      subjectAnalysis, processedData, subjectMapping,
    };
      ...
      <div className="text-center mb-6 pb-4 border-b border-dashed border-gray-300">
        ...
        {!loading && parsedData.length === 0 && !error && (
          <div className="bg-yellow-50 border border-yellow-200 p-4 rounded text-yellow-800">
            <p>No records found in the database for Semester {semesterNumber}.</p>
          </div>
        )}
      </div>
          ...
          <div className="result-analysis-view">
            ...
            <div ref={overallSummaryRef} className="p-4">
              <header className="text-center mb-8">
                <h3 className="text-xl mt-4 pb-1 border-b-2 border-gray-700 inline-block font-semibold text-red-700">RESULT ANALYSIS SUMMARY</h3>
              </header>

              <section className="summary-section mb-8">
                <h3 className="bg-gray-100 p-2 pl-4 mt-6 border-l-4 border-blue-700 text-lg font-semibold text-gray-800">OVERALL RESULT</h3>
                <p>TOTAL STUDENTS APPEARED: <strong>{summary.totalStudents}</strong></p>
                <p>TOTAL STUDENTS (STATISTICALLY PASSED): <strong>{summary.successful}</strong></p>
                <p>OVERALL PASS PERCENTAGE: <strong>{summary.passPercentage}%</strong></p>
              </section>

              <section className="gender-analysis-section mb-8">
                <h3 className="bg-gray-100 p-2 pl-4 mt-6 border-l-4 border-blue-700 text-lg font-semibold text-gray-800">GENDER-WISE ANALYSIS (STATISTICAL)</h3>
                  ...
                  <table className="min-w-full border border-black text-sm text-center">
                    <thead>
                      <tr className="bg-gray-200">
                        ...
                        <th className="p-2 border border-black font-semibold">Male</th>
                        <th className="p-2 border border-black font-semibold">Female</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr>
                        <td className="p-2 border border-black font-semibold text-left">Passed</td>
                        <td className="p-2 border border-black text-green-600 font-bold">{summary.malePassed}</td>
                        <td className="p-2 border border-black text-green-600 font-bold">{summary.femalePassed}</td>
                      </tr>
                      <tr>
                        <td className="p-2 border border-black font-semibold text-left">Failed</td>
                        <td className="p-2 border border-black text-red-600 font-bold">{summary.maleFailed}</td>
                        <td className="p-2 border border-black text-red-600 font-bold">{summary.femaleFailed}</td>
                      </tr>
              ...
              <section className="toppers-section mb-8">
                <h3 className="bg-gray-100 p-2 pl-4 mt-6 border-l-4 border-blue-700 text-lg font-semibold text-gray-800">OVERALL TOPPERS</h3>
                <div className="overflow-x-auto mt-4">
                  <table className="min-w-full border border-gray-300 text-sm">
                    <thead>
                      <tr className="bg-blue-100">
                        <th className="p-2 border border-gray-300 font-semibold text-left">RANK</th>
                        <th className="p-2 border border-gray-300 font-semibold text-left">NAME</th>
                        <th className="p-2 border border-gray-300 font-semibold text-center">SGPI</th>
                      </tr>
                    </thead>
                    <tbody>
                      {summary.overallToppers.map((student, index) => (
                        <tr key={student.Seat_No}>
                          ...
                          <td className="p-2 border text-center">{student.SGPI}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <section className="subject-toppers-section mb-8">
                <h3 className="bg-gray-100 p-2 pl-4 mt-6 border-l-4 border-blue-700 text-lg font-semibold text-gray-800">TOPPER IN SUBJECT (Top Two Scores)</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
                  {Object.entries(summary.subjectToppers).map(([subject, toppers]) => {
                    const rank1Mark = toppers[0]?.marks;
                    const rank2MarkEntry = toppers.find((s) => s.marks < rank1Mark);
                    const rank2Mark = rank2MarkEntry?.marks;

                    return (
                      <div key={subject} className="border border-green-300 rounded-lg shadow-md p-3 bg-white">
                        <h4 className="text-center text-base font-bold text-green-700 border-b pb-1 mb-2">{subject}</h4>
                        <table className="w-full text-xs">
                          <thead>
                            <tr className="bg-green-50">
                              <th className="p-1 border border-gray-300">Rank</th>
                              <th className="p-1 border border-gray-300 text-left">Name</th>
                              <th className="p-1 border border-gray-300">Marks (100)</th>
                            </tr>
                          </thead>
                          <tbody>
                            {toppers.map((student) => {
                              let displayRank: number | string = "";
                              if (student.marks === rank1Mark) displayRank = 1;
                              else if (student.marks === rank2Mark) displayRank = 2;
                              else return null;

                              return (
                                <tr key={student.seatNo} className={displayRank === 1 ? "bg-yellow-50 font-semibold" : "bg-white"}>
                                  <td className="p-1 border border-gray-300 text-center">{displayRank}</td>
                                  <td className="p-1 border border-gray-300 text-left">{student.name}</td>
                                  <td className="p-1 border border-gray-300 text-center">{student.marks.toString()}</td>
                                </tr>
                              );
                            })}
                            {toppers.length === 0 && (
                              <tr><td colSpan={3} className="text-center text-gray-500 p-2">No valid marks recorded.</td></tr>
                            )}
                          </tbody>
                        </table>
                      </div>
            ...
            <section className="detailed-results">
              <h3 className="bg-red-100 p-2 pl-4 mt-6 border-l-4 border-red-700 text-lg font-semibold text-gray-800">DETAILED STUDENT SCORE CARD</h3>
              <div className="overflow-x-auto mt-4">
                <table className="min-w-full border border-gray-300 text-xs text-center">
                  <thead>
                    <tr className="bg-red-50">
                      ...
                      {subjectKeysForTable.map((key) => (<th className="p-2 border" key={key}>{summary.subjectMapping[key]}</th>))}
                  ...
                  <tbody>
                    {summary.processedData.map((student) => (
                      <tr key={student.Seat_No}>
                        ...
                        {subjectKeysForTable.map((key) => (<td className="p-1 border" key={key}>{student[key] || "-"}</td>))}
                        <td className="p-1 border font-semibold">{student.SGPI || "-"}</td>
                        <td className={`p-1 border font-bold ${student.finalStatus === "Passed" ? "text-green-600" : "text-red-600"}`}>{student.finalStatus}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
