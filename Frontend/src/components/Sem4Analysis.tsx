...
const Sem4Analysis = () => {
  ...
  const [selectedBatch, setSelectedBatch] = useState(""); // Catches server smart-default
  const [isLoading, setIsLoading] = useState(true);
  const lastFetchedBatch = useRef(null);

  // Mapped strictly to Sem 4 S.E. IT Theory Subjects
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/students/sem/4${queryParam}`)
      .then((res) => res.json())
      .then((res) => {
        setData(res);
    ...
    return (
      <div className="flex justify-center items-center h-screen bg-slate-50">
        <div className="p-10 text-center text-blue-600 font-bold text-xl animate-pulse">
          Loading Official Gazette (Sem 4)...
        </div>
      </div>
    );
  }
  ...
  // Clean UI mapping for S.E. Semester IV (IT)
  const cleanSubjectNames = {
    EM4_TOT_Marks: "Engg Mathematics-IV",
    CNND_TOT_Marks: "Comp Networks (CNND)",
    OS_TOT_Marks: "Operating System (OS)",
    AT_TOT_Marks: "Automata Theory (AT)",
    COA_TOT_Marks: "Comp Org & Arch (COA)",
  };
        ...
        <>
          ...
          <div className="text-center mb-8 leading-tight">
            ...
            <h3 className="text-xl font-bold text-slate-900">
              RESULT ANALYSIS S.E. SEMESTER IV (BATCH: {selectedBatch})
            </h3>
