...
// import { extractSem1Data, extractSem2Data } from '../utils/pdfExtractors.js';
...
export const uploadCsvData = async (req, res) => {
  try {
    ...
    data.forEach((s) => {
      ...
      const isFail = finalResultStr.toLowerCase().includes('fail') || 
...
export const uploadPdfData = async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ message: "Upload a PDF file." });
    
    const semNum = Number(req.body.semester);
    if (!semNum || (semNum !== 1 && semNum !== 2)) {
        return res.status(400).json({ message: "Currently only Sem 1 and 2 PDFs are supported directly." });
    }
    ...
    extractedData.forEach((s) => {
      ...
      const isFail = finalResultStr.toLowerCase().includes('fail') || finalResultStr.toLowerCase().includes('unsuccessful');
...
// --- 3. Get Students (Frontend Charts/Tables) ---
...
// --- 4. Get Student History (Profile View) ---
...
export const getStudentsByBatch = async (req, res) => {
  try {
    ...
    // --- FIX: Send the structured object the frontend expects ---
    res.status(200).json({
      batch: batch,
