...
const useDownloadReport = (
  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      const analysisHtml = getFullHtmlContent(subjectAnalysisRef, "RESULT ANALYSIS B.E. SEM I (Subjectwise Report)");
      if (analysisHtml) {
        ...
        pdf.addImage(analysisCanvas.toDataURL("image/jpeg", 1.0), "JPEG", 0, currentYPosition, pdfWidth, imgHeight);
        currentYPosition += imgHeight + 10;
      }

      const summaryHtml = getFullHtmlContent(overallSummaryRef, "RESULT ANALYSIS B.E. SEM I (Summary & Toppers)");
      if (summaryHtml) {
        document.body.appendChild(summaryHtml);
        tempElements.push(summaryHtml);
        await new Promise((resolve) => setTimeout(resolve, 100));
        const summaryCanvas = await html2canvas(summaryHtml, html2canvasOptions);
        const imgHeight = (summaryCanvas.height * pdfWidth) / summaryCanvas.width;
        pdf.addImage(summaryCanvas.toDataURL("image/jpeg", 1.0), "JPEG", 0, currentYPosition, pdfWidth, imgHeight);
      }
      ...
      tempElements.forEach((el) => document.body.contains(el) && document.body.removeChild(el));
  ...
  const downloadPng = async () => {
    ...
    try {
      ...
      const page1Html = getFullHtmlContent(subjectAnalysisRef, "RESULT ANALYSIS B.E. SEM I (Subjectwise Report)");
      ...
      const page2Html = getFullHtmlContent(overallSummaryRef, "RESULT ANALYSIS B.E. SEM I (Summary & Toppers)");
...
const ResultUploaderAndViewer: React.FC = () => {
  ...
  const summary: SummaryData = useMemo(() => {
    ...
    const malePassed = parsedData.filter((s) => s.Gender === "Male" && s.Result === "Successful").length;
    const maleFailed = parsedData.filter((s) => s.Gender === "Male" && (s.Result === "Unsuccessful" || s.Result === "ABS")).length;
    const femalePassed = parsedData.filter((s) => s.Gender === "Female" && s.Result === "Successful").length;
    const femaleFailed = parsedData.filter((s) => s.Gender === "Female" && (s.Result === "Unsuccessful" || s.Result === "ABS")).length;
    const totalStudents = parsedData.length;
    const successful = malePassed + femalePassed;
    const passPercentageOverall = totalStudents > 0 ? (successful / totalStudents) * 100 : 0;

    const overallToppers = parsedData
      .filter((s) => s.Result === "Successful" && s.SGPI && !isNaN(parseFloat(s.SGPI)))
    ...
    subjectKeys.forEach((markKey) => {
      ...
      const studentsWithMarks = parsedData.filter((student) => student[markKey] && !isNaN(parseInt(student[markKey])));
      const totalAppeared = studentsWithMarks.length;
      const passMark = subjectName === "Eng. Physics I" || subjectName === "Eng. Chem I" ? 30 : 40;
      const marks40_50 = studentsWithMarks.filter((s) => { const marks = parseInt(s[markKey]); return marks >= 40 && marks <= 50; }).length;
      const marks51_59 = studentsWithMarks.filter((s) => { const marks = parseInt(s[markKey]); return marks >= 51 && marks <= 59; }).length;
      const marks60_Above = studentsWithMarks.filter((s) => { const marks = parseInt(s[markKey]); return marks >= 60; }).length;
      const totalPassed = studentsWithMarks.filter((s) => { const marks = parseInt(s[markKey]); return marks >= passMark; }).length;
    ...
    subjectKeys.forEach((markKey) => {
      const sortedStudents = parsedData
        .filter((student) => student[markKey] && !isNaN(parseInt(student[markKey])))
      ...
      for (const student of sortedStudents) {
        ...
        if (uniqueMarks.size < 2 || uniqueMarks.has(currentMark)) {
          if (toppersList.length === 0 || currentMark === toppersList[0].marks) {
            toppersList.push({ name: student.Name, marks: currentMark, seatNo: student["Seat No"] });
            uniqueMarks.add(currentMark);
          } else if (uniqueMarks.size === 1) {
            toppersList.push({ name: student.Name, marks: currentMark, seatNo: student["Seat No"] });
            uniqueMarks.add(currentMark);
          } else if (uniqueMarks.size === 2 && currentMark === toppersList[toppersList.length - 1].marks) {
            toppersList.push({ name: student.Name, marks: currentMark, seatNo: student["Seat No"] });
            uniqueMarks.add(currentMark);
          }
        }
        if (uniqueMarks.size === 2 && currentMark < toppersList[toppersList.length - 1].marks) break;
      }
      subjectToppers[subjectMapping[markKey]] = toppersList;
    });

    return { totalStudents, successful, passPercentage: passPercentageOverall.toFixed(2), overallToppers, subjectToppers, malePassed, maleFailed, femalePassed, femaleFailed, subjectAnalysis } as SummaryData;
  }, [parsedData]);
      ...
      <div className="text-center mb-6 pb-4 border-b border-dashed border-gray-300">
        ...
        {!loading && parsedData.length === 0 && !error && (
            <div className="bg-yellow-50 border border-yellow-200 p-4 rounded text-yellow-800">
                ...
                <p className="text-sm mt-1">Go to the "Upload PDF" page to add student data first.</p>
            </div>
        )}
        {!loading && parsedData.length > 0 && (
           <p className="text-green-700 font-medium">Successfully loaded {parsedData.length} records.</p>
        )}
      </div>

      {parsedData.length > 0 && (
        <div className="text-center mb-6 space-x-4">
          <button onClick={downloadPdf} disabled={isDownloading} className={`px-6 py-2 rounded-lg text-white font-semibold transition duration-150 ${isDownloading ? "bg-gray-400 cursor-not-allowed" : "bg-red-600 hover:bg-red-700 shadow-lg"}`}>
            {isDownloading ? "Generating PDF..." : "⬇️ Download as PDF"}
          </button>
          <button onClick={downloadPng} disabled={isDownloading} className={`px-6 py-2 rounded-lg text-white font-semibold transition duration-150 ${isDownloading ? "bg-gray-400 cursor-not-allowed" : "bg-green-600 hover:bg-green-700 shadow-lg"}`}>
        ...
        <div className="result-analysis-view">
          ...
          <div ref={overallSummaryRef} className="p-4">
            ...
            <section className="summary-section mb-8">
              ...
              <p className="text-base mt-2">TOTAL NO. OF STUDENTS APPEARED: <strong className="text-blue-800">{summary.totalStudents}</strong></p>
              <p className="text-base">TOTAL NO. OF STUDENTS PASSED: <strong className="text-green-600">{summary.successful}</strong></p>
              <p className="text-base">OVERALL RESULT: <strong className="text-blue-800">{summary.passPercentage}%</strong></p>
            </section>
                ...
                <table className="min-w-full border border-gray-300 text-sm">
                  <thead>
                    <tr className="bg-blue-100">
                      ...
                      <th className="p-2 border border-gray-300 font-semibold text-left">NAME OF THE STUDENT</th>
                      <th className="p-2 border border-gray-300 font-semibold text-center">SGPI (10)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {summary.overallToppers.map((student, index) => (
                      <tr key={student["Seat No"]} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                        <td className="p-2 border border-gray-300 text-left font-bold">{index + 1}</td>
                        <td className="p-2 border border-gray-300 text-left">{student.Name}</td>
                        <td className="p-2 border border-gray-300 text-center">{student.SGPI}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                    ...
                    >
                      ...
                      <td
                        className={`p-2 border border-gray-300 font-bold ${
                          student.Result === "Successful" || student.Result === "P"
                            ? "text-green-600"
                            : "text-red-600"
                        }`}
                      >
