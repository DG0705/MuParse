...
export const SUBJECT_NAMES = [
  ...
  "Engineering Mathematics-I Term Work",
  "Engineering Physics-I",
  "Engineering Physics-I (TW)",
  "Engineering Chemistry-I",
  "Engineering Chemistry-I (TW)",
  "Engineering Mechanics",
  "Engineering Mechanics (TW/OR/PR)",
  "Basic Electrical Engineering",
  "Basic Electrical Engineering (TW/OR/PR)",
  "Basic Workshop Practice-I",
];

const CSV_SUBJECT_HEADERS = [
  "Eng_Maths_I",
  "Eng_Maths_I_TW",
  "Eng_Physics_I",
  "Eng_Physics_I_TW",
...
export function parseStudentsFromTxt(raw: string): StudentRecord[] {
  ...
  for (const block of blocks) {
    ...
    if (bottomGroupLine) {
      ...
      papersBottom = segs.slice(0, 5).map(parseGroup);
    }
    const combinedPapers = [...papersTop, ...papersBottom];
    while (combinedPapers.length < 11) combinedPapers.push({ cr: null, gr: null, gp: null, cxg: null, marks: null });
    ...
    if (bottomMarksLine) {
      ...
      bottomMarksTotals = segs.slice(0, 5).map(parseMarksTotal);
    }
    for (let i = 0; i < Math.min(6, combinedPapers.length); i++) {
      if(combinedPapers[i]) combinedPapers[i].marks = topMarksTotals[i] ?? null;
    }
    for (let i = 0; i < Math.min(5, Math.max(0, combinedPapers.length - 6)); i++) {
      if(combinedPapers[6 + i]) combinedPapers[6 + i].marks = bottomMarksTotals[i] ?? null;
    }
    
    const papers: PaperStats[] = combinedPapers.slice(0, 11).map((p, i) => ({
      name: SUBJECT_NAMES[i] || `Paper ${i + 1}`,
      cr: p.cr,
      gr: p.gr,
      gp: p.gp,
      cxg: p.cxg,
...
export function toCsv(students: StudentRecord[]): string {
  ...
  const rows = students.map((s) => {
      const paperValues = s.papers.slice(0, 11).flatMap((p) => [p.cr ?? "", p.gr ?? "", p.gp ?? "", p.cxg ?? "", p.marks ?? ""]);
